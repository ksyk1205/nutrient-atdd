...
public class SupplementContent {
    ...
    public SupplementContent update(String serialNumber, LocalDate permitDate,
        String expirationDate, String intake, String mainFunctional, String precautions,
        String storageWay) {
        if (!updateCheck(serialNumber, permitDate,
            expirationDate, intake, mainFunctional, precautions, storageWay)) {
            return this;
        }
        return new SupplementContent(serialNumber, permitDate,
            expirationDate, intake, mainFunctional, precautions, storageWay);
    }

    private boolean updateCheck(String serialNumber, LocalDate permitDate, String expirationDate,
        String intake, String mainFunctional, String precautions, String storageWay) {
        String updateSerialNumber = updateCheckSerialNumber(serialNumber);
        LocalDate updatePermitDate = updateCheckPermitDate(permitDate);
        String updateExpirationDate = updateCheckExpirationDate(expirationDate);
        String updateIntake = updateCheckIntake(intake);
        String updateMainFunctional = updateCheckMainFunctional(mainFunctional);
        String updatePrecautions = updateCheckPrecautions(precautions);
        SupplementContent supplementContent = new SupplementContent(updateSerialNumber, updatePermitDate,
            updateExpirationDate, updateIntake, updateMainFunctional, updatePrecautions, storageWay);
        if (this.equals(supplementContent)) {
            return false;
        }
        return true;
    }
    ...
    private LocalDate updateCheckPermitDate(LocalDate permitDate) {
        if (permitDate == null){
